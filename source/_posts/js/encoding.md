...
为了理解 Javascript 语言中对于字符与编码存在的问题和解决方案，我们首先需要了解下编码相关的基本概念。[文章](/2022/03/06/misc/encoding/)介绍了字符集（Character Set）、字符编码（Character Encoding）、码点（Code Point)、编码单元（Code Unit）等概念，以及 UCS2 和 UTF16 两种编码方案的内容与区别。

Javascript 对于编码问题的处理要考虑以下这些问题。
...
源码的 Unicode 码点序列中如果包括组合字符序列（combining character sequence），不会对其进行组合当成一个字符而是对每个字符单独处理。
...
规范 [The String Type](https://tc39.es/ecma262/multipage/ecmascript-data-types-and-values.html#sec-ecmascript-language-types-string-type)规定了 Javascript 中的字符串的内部存储格式是顺序的 16 位的编码单元序列，最长可以包含 2^53 - 1 个编码单元。
...
编码单元序列可以表示整个 Unicode 的编码范围（0x0000 ~ 0x10FFFF），但是在具体的编码含义上跟 UCS2 和 UCS16 都有些区别，并不完全一致。

UCS2 是 16 为定长编码，只能表示基本平面（0x0000 ~ 0xFFFF）内的字符，不能表示补充平面的字符。UTF16 是变长编码，使用一个编码单元表示基本平面内的字符，使用两个编码单元的代理对（Surrogate Pair）表示补充平面字符，但是不允许非法的代理对。

Javascript 中的编码相比于 UCS2 多了代理对的支持，相比于 UTF16 又允许非法的代理对，情况分类如下。

不属于代理对的单个编码单元代表具有相同数值的码点，单个编码单元`\u0061`代表了英文字母`"a"`。

合法的两个连续的编码单元，第一个是高代理对，第二个是低代理对，这两个合起来被认为是相应的码点。`'\uD83D\uDE00'`代表了字符`'😀'`（U+1F600 GRINNING FACE）。

不在合法代理对形式中的单个代理对码点代表具有相同数值的码点，编码单元`'\uD800'`就代表码点`\u{D800}`，只不过这个码点在 Unicode 中没有对应的文字。
...
对于中文等基本平面中的字符，或者 `'😀'`等补充平面中的字符可以直接书写，因为字符串内可以使用符合源码字符（[SourceCharacter](https://tc39.es/ecma262/multipage/ecmascript-language-source-code.html#prod-SourceCharacter)）形式的任意字符，也就是除了单双引号、反斜杠、换行符之外的所有 Unicode 字符。
...
另外可以使用多种形式的转义序列（EscapeSequence）来表示任意 Unicode 字符，关于 Javascript 中转译序列的细节可以参考[文章](https://mathiasbynens.be/notes/javascript-escapes)。
...
对于补充平面中的字符可以使用 Unicode 转义序列代理对的形式书写，也可以使用 Unicode 码点转义序列形式。
...
这两种形式表示的字符是相同的，在字符存储的过程中会被转换为**相同的**内部 16 位编码单元序列。

> When generating these String values Unicode code points are UTF-16 encoded as defined in 11.1.1.

## 相等性

由于 Javascript 内部使用 16 位编码单元序列表示字符串，所以两个字符串等的含义是这两个编码[单元序列完全一致](https://tc39.es/ecma262/multipage/abstract-operations.html#sec-samevaluenonnumeric)，长度相同而且在相同下标出编码单元相同。

对于普通字符串来说这个含义符合我们的预期`'a' === '\u0061'`。

但是对于可以有不同码点序列表示的组合字符来说，同一个字符的不同编码序列不同。`str1`使用了规范化的组合形式（Normalization Form Composition）表示[单个字符](https://codepoints.net/U+00E7)`'ç'`，被保存为`'\u00E7'`；`str2`使用了组合的形式`'c\u0327'`，被保存为`\u0063\u0327`。字符相同，但是相等性判断为假。
...
字符串的长度（[length](https://tc39.es/ecma262/multipage/text-processing.html#sec-properties-of-string-instances-length)）是 16 位编码单元的个数，但是有些额外情况需要考虑，参考文章[1](https://mathiasbynens.be/notes/javascript-unicode#counting-symbols), [2](https://dmitripavlutin.com/what-every-javascript-developer-should-know-about-unicode/#33-string-length)。
...
对于补充平面中的字符来说，一个字符需要两个编码单元表示，为了获得预期的字符个数，需要将一个代理对当成一个字符处理。

可以使用 [punycode](https://github.com/mathiasbynens/punycode.js) 库。
...
也可以使用[string's iterator](https://mathiasbynens.be/notes/javascript-unicode#iterating-over-symbols)来获得字符串的码点序列，然后返回码点序列的码点个数。
...
这个做法很浪费内存，如果只是需要直到长度的话，使用 punycode 库
...
但不是所有的组合字符序列代表的单个字符都有对应的经典形式（Canonical Form），例如字符`'ȩ́'`包含了两个组合字符（Combining Mark）`\u0327\u0301`，正规化的结果只能表示为`'ȩ\u0301'`，所以正规化后的字符串长度是 5。
...
考虑补充平面字符和组合字符同时存在的情况，可以先进行规范化，然后获取码点个数。
...
更夸张的情况是泰文中可以使用任意长度的组合字符，出现比较奇葩的效果。
...
可以参考 UAX 中关于[字符边界的算法](http://www.unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries)

## String.prototype 对象方法

### charAt & charCodeAt & fromCharCode

`chartCodeAt`方法获取指定下标的编码单元，返回结果是数字；`charAt` 方法获取字符串在指定下标的编码单元对应的字符，返回结果是字符串。这两个方法查询的下标对应基本平面内字符时结果正确，对应代理对时结果将代理对拆分，单个处理。
...
[`fromCharCode`](https://tc39.es/ecma262/multipage/text-processing.html#sec-string.fromcharcode)从给定编码单元值创建对应字符，超过 0xFFFF 的会被截断。
...
字符串存储为 16 为编码单元，所以遍历字符串编码单元非常简单。遍历字符串内的码点序列需要处理代理对，可以手动实现这个逻辑。
...
全局字符串对象原型`String.prototype`上的[其他操作](https://tc39.es/ecma262/multipage/text-processing.html#sec-properties-of-the-string-prototype-object)也都是将 16 位编码单元进行处理的，也会因为补充平面字符、组合字符、字符边界等情况出现与预期不一致的结果，在使用时需要注意。

## 形式转换

字符串的两个形式 16 位编码单元和码点序列（Code Points）在需要的时候进行转换，[规范](https://tc39.es/ecma262/multipage/ecmascript-language-source-code.html#sec-source-text)中使用了以下抽象操作描述正反向的转换过程。

码点序列转换为 16 位编码单元序列，`CodePointsToString(codePoints)`将的码点序列`codePoints`转换为 16 位编码单元序列序列，内部调用`UTF16EncodeCodePoint(cp)`将单个码点`cp`转换为 16 位编码单元代表的字符串。

16 为编码单元序列转换为码点序列，`StringToCodePoints(string)`将`string`转换为对应的码点序列，内部调用`CodePointAt(string, position)`获取位于位置`position`的码点，调用 `UTF16SurrogatePairToCodePoint(lead, trail)`将内部的 16 位编码单元序列中代理对转换为对应码点，返回数字类型。

## 反转字符串

对于基本平面中的非组合字符反转字符串非常简单。
...
但是对于组合字符的情况，位置反转会造成组合字符修饰的字符不同，所以反转结果与预期不一致。

```js
;[...'mañana'].reverse().join('') // "anãnam"
```

综合考虑以上情况，可以使用[esrever](https://github.com/mathiasbynens/esrever)库来处理代理对、组合字符串等问题。

https://mathiasbynens.be/notes/javascript-unicode#counting-symbols#reversing-strings
...
[String.raw](https://mothereff.in/js-escapes#1%F0%9D%8C%86)是模板字符串标签函数，用来可以保留模板字符串中的转义序列。
...
正则表达式默认也是使用 16 位编码单元处理字符匹配，因此在处理补充平面字符时可能出现与预期不一致的情况。
...
为了 16 位编码单元序列代表的所有合法 Unicode 字符串序列，可以编写特定模式进行匹配，将 Unicode 字符串分为四类。
...
使用 ES6 新增的正则表达式标志 u 开启 Unicode 支持。
...
如果需要在 ES5 等环境中进行兼容处理，同样可以使用 regenerate 库生成相应匹配形式。
...
对 Javascript 涉及的字符串与编码问题可以首先阅读以下内容。
